/* 
 * File:   SimpleCash1Test.cpp
 * Author: raitis
 *
 * Created on August 4, 2018, 5:34 PM
 */
...
template<typename T>
void simpleCash1_take_test(
        SimpleCash1<T>& cash,const T& vect, 
...
std::pair<double, double> result = cash.take(vect);
    if (expected.first != result.first ||
        ...
        std::cout << "%TEST_FAILED% time=0 testname=simpleCash1_take_test (SimpleCash_Test) message=Result differ" << std::endl;
    }
    if(expected_size> -1){
    if(cash.size()!=expected_size){
         std::cout << "%TEST_FAILED% time=0 testname=simpleCash1_take_test (SimpleCash_Test) message=Bad cash size! expected= "
...
void testTake() {
    SimpleCash1<Vect3DCsh> simpleCash;
    Vect3DCsh vectors[]={
    ...
    for(int i=0; i< sizeof(vectors)/sizeof(Vect3DCsh); i++){
     ...
     simpleCash1_take_test(simpleCash,vectors[i],expected[i], -1 );
    ...
    if (simpleCash.size() != 3) {
        std::cout << "%TEST_FAILED% time=0 testname=testTake (SimpleCash1Test) message=error size not 3!" << std::endl;
    }
}

template <typename T>
void thread_test_task_1(SimpleCash1<T>& cash){
    ...
    for(int i=0; i< sizeof(vectors)/sizeof(Vect3DCsh); i++){
         simpleCash1_take_test(cash,vectors[i],expected[i], -1 );
    }
...
template <typename T>
void random_gen_task(SimpleCash1<T>& cash, int vectNums){
...
int main(int argc, char** argv) {
    std::cout << "%SUITE_STARTING% SimpleCash1Test" << std::endl;
    std::cout << "%SUITE_STARTED%" << std::endl;

    std::cout << "%TEST_STARTED% testTake (SimpleCash1Test)" << std::endl;
    testTake();
    std::cout << "%TEST_FINISHED% time=0 testTake (SimpleCash1Test)" << std::endl;

    std::cout << "%TEST_STARTED% test_thread_test_task_1 (SimpleCash1Test)" << std::endl;
    test_thread_test_task_1();
    std::cout << "%TEST_FINISHED% time=0 test_thread_test_task_1 (SimpleCash1Test)" << std::endl;

     std::cout << "%TEST_STARTED% test_random_gen_tasks (SimpleCash1Test)" << std::endl;
     test_random_gen_tasks();
    std::cout << "%TEST_FINISHED% time=0 test_random_gen_tasks (SimpleCash1Test)" << std::endl;
